...
vpd_mon = vpd_mon(:,k); SPI = SPI(:,k); tws = tws(:,k); vpdz = vpdz(:,k);
time = time(k);
%% Calculate Z score
% kick out the edges
load kland_025.mat
...
for J = 1:4
    ...
    if J == 3; spi_thres = -1; vpd_thres = 0; end
    if J == 4; spi_thres = -1; vpd_thres = 1; end


    [et_change] = cal_water_storage_change_warm(tws_detrend, SPI, time, lats, vpdz, vpdz,spi_thres, vpd_thres,20);
     dsd_all(:,J) = et_change;
end
...
dsums = [nanmean(dsums_cp)' nanmean(dsums_fre)' nanmean(dsums_no_cp_fre)'];
dsum_stds = [range(dsums_cp)' range(dsums_fre)' range(dsums_no_cp_fre)']/2;
...
title('(a) E(TWS | Z_p<0, Z_{VPD}>0)', 'FontWeight', 'bold', 'FontName', 'Arial', 'FontSize', 14);
...
hh(3).FaceColor = [0.45, 0.75, 0.45]; % Soft green (e.g., forest/other vegetation)
...
xlim([0.5 4.5]);
xticks(x);
xticklabels({'Z_P<0', 'Z_{VPD}>0', 'Z_P<0 & Z_{VPD}>1', 'Z_P<-1 & Z_{VPD}>0', 'Z_P<-1 & Z_{VPD}>1'});
...
title('(b) TWS during droughts', 'FontWeight', 'bold', 'FontSize', 14, 'FontName', 'Arial');
set(gca, 'YTick', -3:1:0);
box on;
...
% Title and final touches
title('(c) Total TWS reduction during droughts', 'FontWeight', 'bold', 'FontSize', 14, 'FontName', 'Arial');
% set(gca, 'YTick', -3:1:0);
box on;
...
set(h1, 'Position', [0.015 0.5 0.9 0.5]);
set(c1, 'Position', [0.015+.22 0.57 0.5 0.01]);

set(h2, 'Position', [0.1 0.31 0.75 0.18]);

set(h3, 'Position', [0.1 0.05 0.75 0.18]);
...
filename='./Figures/FigureS_drought_impacts_TWS_All_zsocre.png'; 
