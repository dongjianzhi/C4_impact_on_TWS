...
vpdz = vpdz(:,k); SPI = SPI(:,k); tws = tws(:,k);
time = time(k);
%% Calculate Z score
% kick out the edges
load kland_025.mat
...
% ({'Z_P<0', 'Z_{VPD}>0', 'Z_P<0 & Z_{VPD}>1', 'Z_P<-1 & Z_{VPD}>0', 'Z_P<-1 & Z_{VPD}>1'});
...
title('(a) E(TWSd | Z_p<0, Z_{VPD}>0)', 'FontWeight', 'bold', 'FontName', 'Arial', 'FontSize', 14);
...
xlim([0.5 4.5]);
xticks(x);
xticklabels({'Z_P<0', 'Z_{VPD}>0',  'Z_P<-1 & Z_{VPD}>0', 'Z_P<-1 & Z_{VPD}>1'});
...
filename='./Figures/Figure1_drought_impacts_TWS_upd_vpd.png'; 
